# App ID Ranges

| App Name                     | ID Range        |
|------------------------------|-----------------|
| walter75 - BaseApp Basic     | 80000–80099     |
| walter75 - Report Extensions | 80100–80199     |
| walter75 - OAuth 2.0         | 90000–90099     |
| walter75 - BDE Terminal      | 90600–90699     |
| walter75 - Packages          | 90700–90799     |
| walter75 - Contact Relations | 91300–91399     |
| walter75 - Freight Prices    | 91400–91499     |
| walter75 - Color Master      | 91600–91699     |
| walter75 - Lead Management   | 91700–91799     |
| walter75 - PrintNode         | 92700–92799     |
| walter75 - SendCloud         | 95700–95799     |
| walter75 - XML Import        | 60500–60799     |
| walter75 - Calculation       | 90800-90899     |


**Hinweis:** Die ID-Ranges sind exklusiv pro App und dürfen nicht überschritten oder mehrfach verwendet werden. Immer vor dem Anlegen neuer Objekte die jeweilige `app.json` prüfen.

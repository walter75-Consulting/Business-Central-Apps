page 90710 "SEW Packing Card FB Details"
{
        ...
        area(Content)
        {
            group(Source)
            {
                ...
                field("Source Document"; Rec."Source Document")
                {
                    ...
                    ToolTip = 'Specifies the type of document that the warehouse activity line is created for.';
                }
                field("Source No."; Rec."Source No.")
                {
                    ApplicationArea = Warehouse;
                    ToolTip = 'Specifies the number of the document that the warehouse activity line is created for.';
                }
            }
            group(Destination)
            {
                ...
                {
                    ApplicationArea = Warehouse;
                    CaptionClass = Format(WMSManagement.GetCaptionClass(Rec."Destination Type", Rec."Source Document", 0));
                    Editable = false;
                    ToolTip = 'Specifies the number or the code of the customer or vendor that the line is linked to.';
                }
                field(DestinationEntityName; WMSManagement.GetDestinationEntityName(Rec."Destination Type", Rec."Destination No."))
                {
                    ApplicationArea = Warehouse;
                    CaptionClass = Format(WMSManagement.GetCaptionClass(Rec."Destination Type", Rec."Source Document", 1));
                    Caption = 'Name';
                    Editable = false;
